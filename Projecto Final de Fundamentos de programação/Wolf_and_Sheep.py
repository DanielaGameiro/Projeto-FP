import pygame
import os
from pygame.locals import*
import pygame.freetype

def Menu():

    pygame.init()
    pygame.font.init()

    cumprimento = 1300
    largura = 650
    screen = pygame.display.set_mode((cumprimento, largura))

    pygame.display.set_caption("Wolf & Sheep")
    icon = pygame.image.load('Wolf_&_Sheep_v2.png')
    pygame.display.set_icon(icon)

    Sheep = pygame.image.load('Sheep.png').convert_alpha()
    Sheep_x = 1000
    Sheep_y = 0

    Wolf = pygame.image.load('Wolf.png').convert_alpha()
    Wolf_x = 20
    Wolf_y = 0

    # my_font = pygame.freetype.Font("NotoSans-Regular.ttf", 100)
    # my_font.render_to(screen, (20, 20), "Hell in heaven", (255, 255, 255))

    title = pygame.font.SysFont("NotoSans-Regular.ttff", 100)
    title_surface = title.render("Wolf & Sheep", True, (255, 255, 255))

    start = pygame.font.SysFont("NotoSans-Regular.ttff", 70)
    start_surface = start.render("Start", True, (255, 255, 255))

    leave = pygame.font.SysFont("NotoSans-Regular.ttff", 70)
    leave_surface = leave.render("Exit", True, (255, 255, 255))

    rules = pygame.font.SysFont("NotoSans-Regular.ttff", 70)
    rules_surface = rules.render("Rules", True, (255, 255, 255))

    pygame.mixer.music.load('non-copyrighted-music-lakey-inspired-chill-day-hip-hop-beat.ogg')
    pygame.mixer.music.play(0)

    clock = pygame.time.Clock()
    running = True

    while (running):

        screen.fill((214, 162, 94))

        rect1_exterior = pygame.draw.rect(screen, (223, 0, 100), (550, 250, 200, 60), 0)
        rect1_interior = pygame.draw.rect(screen, (255, 138, 190), (562, 255, 175, 50), 0)
        rect2_exterior = pygame.draw.rect(screen, (223, 0, 100), (550, 450, 200, 60), 0)
        rect2_interior = pygame.draw.rect(screen, (255, 138, 190), (562, 455, 175, 50), 0)
        rect3_exterior = pygame.draw.rect(screen, (223, 0, 100), (550, 350, 200, 60), 0)
        rect3_interior = pygame.draw.rect(screen, (255, 138, 190), (562, 355, 175, 50), 0)


        for event in pygame.event.get():
            if (event.type == pygame.QUIT):
                running = False

        if (event.type == pygame.MOUSEBUTTONDOWN):
            mx, my = pygame.mouse.get_pos()
            if (mx > 550) and (mx < 750) and (my > 250) and (my < 310):
                import Game

            if (mx > 550) and (mx < 750) and (my > 350) and (my < 410):
                import Rules

            if (mx > 550) and (mx < 750) and (my > 450) and (my < 510):
                exit()

        screen.blit(Sheep, (Sheep_x, Sheep_y))
        screen.blit(Wolf, (Wolf_x, Wolf_y))
        screen.blit(title_surface,(430, 100))
        screen.blit(start_surface,(590, 260))
        screen.blit(leave_surface,(595, 460))
        screen.blit(rules_surface,(580, 360))
        pygame.display.flip()
        clock.tick(60)
        pygame.display.update()

Menu()